...
void STrafficLightPreviewViewport::Construct(const FArguments &InArgs) {
  ...
  ViewportClient = MakeShareable(
      new FEditorViewportClient(nullptr, PreviewScene.Get(), nullptr));
...
FVector2D STrafficLightPreviewViewport::GetAtlasCoordsForLightType(
    ETLLightType LightType) const {
  if (LightTypesTable == nullptr) {
  ...
  const FString EnumName =
      EnumPtr->GetNameStringByValue(static_cast<int64>(LightType));
...
STrafficLightPreviewViewport::AddModuleMesh(const FTLHead &Head,
                                            FTLModule &ModuleData) {
  const FTransform ModuleWorldTransform{ModuleData.Transform * Head.Transform *
                                        ModuleData.Offset};

  UWorld *World{PreviewScene->GetWorld()};
  UObject *LevelOuter{World->PersistentLevel};
  UStaticMeshComponent *Comp{
      NewObject<UStaticMeshComponent>(LevelOuter, NAME_None, RF_Transient)};
  if (!Comp) {
    UE_LOG(LogTemp, Error,
           TEXT("Failed to create StaticMeshComponent for module"));
    return nullptr;
  }
  ...
  if (LightMID) {
    LightMID->SetScalarParameterValue(TEXT("Emmisive Intensity"),
                                      ModuleData.EmissiveIntensity);
    LightMID->SetVectorParameterValue(TEXT("Emissive Color"),
...
void STrafficLightPreviewViewport::ResetFrame(
  ...
  const FVector Forward{
      FVector::ForwardVector.Rotation().RotateVector(FVector(0, 1, 0))};
