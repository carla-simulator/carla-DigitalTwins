...
void STrafficLightToolWidget::Rebuild()
{
	...
	for (int32 PoleIndex{0}; PoleIndex < EditorPoles.Num(); ++PoleIndex)
	{
		...
		{
			for (int32 ModuleIndex{0}; ModuleIndex < EP.Heads[HeadIndex].Modules.Num(); ++ModuleIndex)
			{
...
void STrafficLightToolWidget::RefreshPoleList()
{
	...
	for (int32 PoleIndex{0}; PoleIndex < EditorPoles.Num(); ++PoleIndex)
	{
		PoleListContainer->AddSlot().AutoHeight().Padding(2, 2)[BuildPoleEntry(PoleIndex)];
	}
}
...
void STrafficLightToolWidget::RefreshModuleMeshOptions(int32 PoleIndex, int32 HeadIndex, int32 ModuleIndex)
{
	...
	TArray<UStaticMesh*> Meshes = FTLMeshFactory::GetAllMeshesForModule(Head.Data, Module.Data);
...
FReply STrafficLightToolWidget::OnAddHeadClicked(int32 PoleIndex)
{
	...
	Rebuild();
	if (NewHead.Modules.Num() > 0 && NewHead.Modules[0].Data.ModuleMeshComponent)
	{
...
FReply STrafficLightToolWidget::OnAddModuleClicked(int32 PoleIndex, int32 HeadIndex)
{
	...
	NewEditorMod.Data.ModuleMesh = FTLMeshFactory::GetMeshForModule(Head.Data, NewEditorMod.Data);
	...
	UStaticMeshComponent* NewComp{PreviewViewport->AddModuleMesh(Head.Data, NewEditorMod.Data)};
...
void STrafficLightToolWidget::OnHeadOrientationChanged(ETLOrientation NewOrientation, int32 PoleIndex, int32 HeadIndex)
{
	...
	{
		UStaticMesh* NewMesh{FTLMeshFactory::GetMeshForModule(EditorHead.Data, EditorModule.Data)};
		if (!IsValid(NewMesh))
		{
			UE_LOG(LogTemp, Error, TEXT("OnHeadOrientationChanged: failed to get mesh"));
			return;
		}
...
TSharedRef<SWidget> STrafficLightToolWidget::BuildPoleEntry(int32 PoleIndex)
{
	...
	auto ExtendiblePoleMeshes{FTLMeshFactory::GetAllExtendibleMeshesForPole(PoleData)};
...
TSharedRef<SWidget> STrafficLightToolWidget::BuildPoleSection()
{
	...
	return SNew(SVerticalBox) +
		   ...
		   SVerticalBox::Slot().FillHeight(1.0f).Padding(
			   10)[SNew(SScrollBox) + SScrollBox::Slot()[PoleListContainer.ToSharedRef()]];
}
...
TSharedRef<SWidget> STrafficLightToolWidget::BuildHeadSection(int32 PoleIndex)
{
	...
	Container->AddSlot().FillHeight(1.0f).Padding(
		2)[SNew(SScrollBox) + SScrollBox::Slot()[SAssignNew(Pole.Container, SVerticalBox)]];
	...
	for (int32 HeadIndex{0}; HeadIndex < Pole.Heads.Num(); ++HeadIndex)
	{
		...
		Pole.Container->AddSlot().AutoHeight().Padding(4, 2)[BuildHeadEntry(PoleIndex, HeadIndex)];
	}
	...
	return SNew(SBorder).Padding(
		4)[SNew(SHorizontalBox) +
			   ...
			   [SNew(SVerticalBox) +
				   SVerticalBox::Slot()
					   .AutoHeight()[SNew(SButton)
										 ...
										 .OnClicked(this, &STrafficLightToolWidget::OnMoveModuleUpClicked, PoleIndex, HeadIndex,
											 ModuleIndex)[SNew(SImage).Image(FAppStyle::Get().GetBrush("Icons.ArrowUp"))]] +
				   SVerticalBox::Slot()
					   .AutoHeight()[SNew(SButton)
										 .ButtonStyle(FAppStyle::Get(), "SimpleButton")
										 .ContentPadding(2)
		   ...
		   SHorizontalBox::Slot().FillWidth(1).VAlign(VAlign_Center)[SNew(SVerticalBox) +
																			   ...
																			   SHorizontalBox::
																				   Slot()
																					   ...
																					   .Padding(8, 0)[SNew(SComboBox<
																										  ...
																										  .OnSelectionChanged_Lambda(
																											  [this,
																												  PoleIndex, HeadIndex, ModuleIndex, &Head](TSharedPtr<FString> NewSel, ESelectInfo::Type)
																											  {
																												  if (!NewSel)
																												  {
																													  return;
																	 ...
																	 + SVerticalBox::
																		   Slot()
																			   ...
																			   .Padding(0, 2)[SNew(SHorizontalBox)
																							  ...
																							  // Label
																							  + SHorizontalBox::Slot().AutoWidth().VAlign(VAlign_Center)[SNew(STextBlock)
																																							 .Text(
																																								 FText::FromString(
																																									 "Light Type:"))]
																							  ...
																							  // Combo
																							  +
																							  SHorizontalBox::Slot().AutoWidth().Padding(8, 0)[SNew(
																								  SComboBox<TSharedPtr<FString>>)
																																				   .OptionsSource(
																																					   &LightTypeOptions)
																																				   .InitiallySelectedItem(
																																					   LightTypeOptions[static_cast<int32>(ModuleData
																																															   .LightType)])
																																				   .OnGenerateWidget_Lambda(
																																				   ...
																																				   .OnSelectionChanged_Lambda(
																																					   [&](TSharedPtr<FString> NewSel, ESelectInfo::Type)
																																					   {
																																						   const int32 Choice{
																																							   LightTypeOptions
																																								   .IndexOfByPredicate(
																																								   ...
																																								   ->LightTypesTable)
																																						   {
																																							   ...
																																							   else
																																							   {
																																								   UE_LOG(
																																									   ...
																																									   Warning,
																																									   TEXT(
																																										   "LightTypesTable: row not found '%s'"),
																																									   *RowName
																																											.ToString());
																																							   }
																																						   }
																	 ...
																	 +
																	 SVerticalBox::Slot().AutoHeight().Padding(0, 2)[SNew(
																														 SHorizontalBox)
																		 ...
																		 2)[SNew(SCheckBox)
																				.IsChecked_Lambda(
																					[this, PoleIndex, HeadIndex, ModuleIndex,
																						&Head]() {
																						return Head.Modules[ModuleIndex]
																									   .Data.bHasVisor
																								   ? ECheckBoxState::Checked
																								   : ECheckBoxState::Unchecked;
																					})
	...
	for (int32 ModuleIndex{0}; ModuleIndex < Head.Modules.Num(); ++ModuleIndex)
	{
		...
		Container->AddSlot().AutoHeight().Padding(
			4, 2)[SNew(SExpandableArea)
					  ...
					  .Padding(4)
					  .OnAreaExpansionChanged_Lambda([this, PoleIndex, HeadIndex, ModuleIndex](bool bExpanded)
						  { EditorPoles[PoleIndex].Heads[HeadIndex].Modules[ModuleIndex].bExpanded = bExpanded; })
